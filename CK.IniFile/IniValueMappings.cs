...
namespace CK.IniFile
{
    ...
    public sealed class IniValueMappings
    {
        ...
        internal IniValueMappings() { mappings = new Dictionary<string, object>( StringComparer.OrdinalIgnoreCase ); }
        ...
        public void Add<T>( string value, T mappedResult )
        {
            ...
            if( Contains( value ) ) throw new InvalidOperationException();

            mappings.Add( value, mappedResult );
        }
        ...
        public bool Contains( string value ) { return mappings.ContainsKey( value ); }
        ...
        public bool Remove( string value ) { return mappings.Remove( value ); }

        internal bool TryGetResult<T>( string value, out T result )
        {
            if( !string.IsNullOrEmpty( value ) )
            {
                if( mappings.TryGetValue( value, out object mappedResult ) &&
