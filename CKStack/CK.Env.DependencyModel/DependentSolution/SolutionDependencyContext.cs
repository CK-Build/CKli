...
namespace CK.Env.DependencyModel
{
    ...
    /// </summary>
    public sealed partial class SolutionDependencyContext
    {
        ...
        public DependentSolution? this[string name] => _index?.GetValueOrDefault( name );
        ...
        /// <returns>The dependent solution or null.</returns>
        public DependentSolution? this[ISolution s] => _index?.GetValueOrDefault( s );
        ...
        /// <summary>
        /// Creates a <see cref="ProjectsDependentInfo"/>. Used to "localize" solutions.
        /// </summary>
        /// <param name="artifactType">The type of artifacts to consider.</param>
        /// <param name="scope">The set of solutions that need project dependencies information.</param>
        /// <returns>The projects dependency info.</returns>
        public ProjectsDependentInfo GetProjectsDependencyInfo( ArtifactType artifactType, IEnumerable<ISolution> scope )
        {
            Throw.CheckArgument( scope.All( s => s.Solutions == _analyzer.Solutions ) );
            var cScope = scope.ToArray();
            // Closure of all the dependent solutions.
            var closure = new HashSet<ISolution>( cScope.Select( s => this[s] )
                                                        .SelectMany( d => d!.TransitiveRequirements )
                                                        .Select( d => d.Solution ) );
            // Map for PackageReference: from PackageId to Project. This is used to transform the <PackagReference>
            // into <ProjectReferences> to the stack local projects.
            // References that are not generated by the closure are not touched.
            var map = closure.SelectMany( d => d.Solution.GeneratedArtifacts.Where( g => g.Artifact.Type == artifactType ) )
                             .ToDictionary( g => g.Artifact, g => (IProject)g.Project );
            return new ProjectsDependentInfo( artifactType, cScope, closure, map );
