...
public static class ProviderDetector
{
    ...
    public static async Task<IGitHostingProvider?> ResolveProviderAsync(
        IActivityMonitor monitor,
        ISecretsStore secretsStore,
        string remoteUrl,
        CancellationToken ct = default )
    {
        var host = RemoteUrlParser.GetHost( remoteUrl );
        ...
        using( monitor.OpenInfo( $"Detecting Git hosting provider for '{host}'..." ) )
        {
            ...
            var parsed = RemoteUrlParser.ParseStandardPath(
                RemoteUrlParser.TryNormalizeToHttps( remoteUrl )?.AbsolutePath ?? "" );
            if( parsed == null )
            {
    ...
    /// Use this when you know the provider type.
    /// </summary>
    /// <param name="providerType">The type of provider to create ("github", "gitlab", or "gitea").</param>
    /// <param name="host">The hostname.</param>
    /// <param name="secretsStore">The secrets store for credential retrieval.</param>
    /// <returns>The created provider, or null if the provider type is unknown.</returns>
    public static IGitHostingProvider? CreateProvider(
        string providerType,
        string host,
        ISecretsStore secretsStore )
    {
