...
sealed partial class DirectPublisher
{
    ...
    {
        var p = new Publisher( this, packageSender, artifactHandler );
        return p.RunAsync( monitor, cancellation );
    }

    /// <summary>
    /// Implements simple, sequential, publication process.
    /// </summary>
    sealed partial class Publisher
    {
        ...
        public Publisher( DirectPublisher state,
                                PackageSender packageSender,
                                ArtifactHandlerPlugin artifactHandler )
        {
            _state = state;
            ...
            while( !step.IsEndOfState )
            {
                switch( step.Location )
                {
                    ...
                    case Cursor.LocType.EndOfRepo:
                        ...
                        break;
                    case Cursor.LocType.EndOfWorld:
        ...
        async Task<Cursor?> OnBegOfRepoAsync( IActivityMonitor monitor, RepoInfo repo, CancellationToken cancel )
        {
        ...
        async Task<Cursor?> OnInPackagesAsync( IActivityMonitor monitor, RepoInfo repo, CancellationToken cancel )
        {
        ...
        async Task<Cursor?> CreateReleaseAsync( IActivityMonitor monitor, RepoInfo repo, int forwardLength, CancellationToken cancel )
        {
        ...
        async Task<Cursor?> OnInFilesAsync( IActivityMonitor monitor, RepoInfo repo, CancellationToken cancel )
        {
        ...
        async Task<Cursor?> OnEndOfRepoAsync( IActivityMonitor monitor, RepoInfo repo, CancellationToken cancel )
        {
        ...
        async Task<Cursor?> OnEndOfWorldAsync( IActivityMonitor monitor, CancellationToken cancel )
        {
            monitor.Info( $"Published '{_state.World.Name}'." );
            return _state.ForwardPrimaryCursor( monitor, 1 );
        }
