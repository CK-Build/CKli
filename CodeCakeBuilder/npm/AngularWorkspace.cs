...
namespace CodeCake
{
    public class AngularWorkspace : NPMProjectContainer
    {
        ...
        public static AngularWorkspace Create( IActivityMonitor m, StandardGlobalInfo globalInfo, NPMSolution npmSolution, NormalizedPath path )
        {
            ...
            foreach( var project in jsonProject.Properties() )
            {
                ...
                {
                    m.Warn( $"No path found for angular project '{path}'." );
            ...
            return new AngularWorkspace( NPMPublishedProject.Create( m, globalInfo, npmSolution, path, path ), projects );
        }
    }
}
