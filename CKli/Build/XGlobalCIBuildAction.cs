...
namespace CKli
{
    public class XGlobalCIBuildAction : XAction
    {
        ...
        public override bool Run( IActivityMonitor m )
        {
            ...
            Console.Write( "Enter http://cksetup.invenietis.net/ key to push components: " );
            string storePushKey = Console.ReadLine();
            if( !String.IsNullOrEmpty( storePushKey ) )
            {
                environmentVariables.Add( ("CKSETUP_CAKE_TARGET_STORE_APIKEY_AND_URL", storePushKey+"|http://cksetup.invenietis.net") );
            }
            ...
            else
            {
                using( m.OpenInfo( "Solutions to build:" ) )
                {
                    ...
                    foreach( var build in list.Skip( startAt ) )
                    {
                        ...
                        {
                            Console.WriteLine( "Hit 'c' to continue. Other kety to stop." );
                            if( Console.ReadKey().KeyChar != 'c' ) return true;

                            if( build.HasAtLeastOneTargetProject )
                            {
                                var toUpgrade = build.Rows.Select( d => (Row: d, Version: _localPackages.GetLocalLastVersion( m, d.Target.Name, false )) )
                                                          .ToList();
